...
<div class="col-span-2 flex flex-none items-center gap-1 bg-gray-100 dark:bg-gray-800 dark:text-gray-300 h-[45px] select-none">
  {#if store.sketchBeingEdited !== ""}
    {#each sketchActions as action}
      <button class="inline-flex items-center p-1 {store.sketchTool === action.alt ? 'bg-gray-400' : 'hover:bg-gray-200'} p-1" onclick={action.handler}>
        <img class="h-8 w-8" src={action.src} alt={action.alt} />{action.text ? action.text : ""}
      </button>
    {/each}
  {:else}
    {#each actions as action}
      <button class="inline-flex items-center hover:bg-gray-200 p-1 rounded dark:hover:bg-gray-600 dark:hover:text-gray-300" onclick={action.handler}>
        <img class="h-8 w-8" src={action.src} alt={action.alt} />{action.text ? action.text : ""}
      </button>
    {/each}
  {/if}
  ...
  {#if debugging}
    Selecting For [
    {#each store.selectingFor as sf}
      <div>
        {sf},
      </div>
    {/each}
    ] Currently Selected [
    {#each store.currentlySelected as cs}
      <div>
        {cs.type}
        {cs.id},
      </div>
    ...
    {#each store.currentlyMousedOver as cm}
      <div>
        {cm.type}
        {cm.id},
      </div>
    ...
    {#each store.hiddenSketches as hs}
      <div>
        {hs},
      </div>
    {/each}
